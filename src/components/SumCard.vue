        ...
        <el-row v-for="(c,index) in list" :key="index">
          <el-card :body-style="{ padding: '0px' }" :key="c.key">
            <div style="padding: 14px;" class="info">
              <div class="row">
                总实验架次数: <span>{{c.flightCounts}}</span>
              </div>
              <div class="row">
                完成的实验点数: <span>{{c.pointsCounts}}</span>
              </div>
              <div class="row">
                总开关车实验时间: <span>{{c.totalTurnOnAndOffTime}}</span>
              </div>
              <div class="row">
                总空中飞行时间: <span>{{c.totalFlightTime}}</span>
              </div>
